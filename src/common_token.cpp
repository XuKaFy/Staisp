...
void Token::print_message(String type, int id, String object,
                          String error_code) {
    ...
    printf("%s:%d:%llu: [%s] %s %d: %s\n", p_code->file_name.c_str(), line,
           (unsigned long long)(token_begin - line_begin + 1), object.c_str(),
...
void Token::throw_error(int id, String object, String error_code) {
    print_message("error", id, object, error_code);
    throw Exception(id, object, error_code);
}

void Token::print_warning(int id, String object, String error_code) {
