...
Token::Token(pCode code, String::iterator token_begin,
    ...
    : p_code(std::move(std::move(code))), token_begin(token_begin), token_end(token_end), line(line) {
}
...
void Token::print_message(const String& type, int id, const String& object,
    ...
    printf("%s:%d:%llu: [%s] %s %d: %s\n", p_code->file_name.c_str(), line,
           static_cast<unsigned long long>(token_begin - line_begin + 1), object.c_str(),
           type.c_str(), id, error_code.c_str());
...
void Token::throw_error(int id, const String& object, const String& error_code) {
    print_message("error", id, object, error_code);
    throw Exception(id, object, error_code);
}

void Token::print_warning(int id, const String& object, const String& error_code) {
