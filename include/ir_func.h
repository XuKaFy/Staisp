...
struct Func : public User {
    Func(TypedSym var, Vector<pType> arg_types, bool variant_length = false)
        : User(make_function_type(var.ty, arg_types)), variant_length(variant_length) {
        set_name(var.sym);
    }
    ...
    Symbol print_func_declaration() const {
        String whole_function = String("declare ") 
            + ty->type_name() + " @" + name() + "("; // functions are all global
        auto func_ty = functon_type();

        if(func_ty->arg_type.empty()) goto PRINT_IMPL_END;
...
pFunc make_func(TypedSym var, Vector<pType> arg_types, bool variant_length = false);
