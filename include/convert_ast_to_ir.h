...
public:
    ...
    static Ir::BinInstrType fromBinaryOpr(const Pointer<Ast::BinaryNode>& root,
                                          const pType& ty);
    static Ir::CmpType fromCmpOpr(const Pointer<Ast::BinaryNode>& root, const pType& ty);
...
private:
    ...
    Ir::pVal analyze_left_value(const pNode& root, bool request_not_const = false);
    ...
    Value from_array_def(const Pointer<Ast::ArrayDefNode>& n, const Pointer<ArrayType>& t);
    ...
    void add_initialization(const TypedSym& var, const Pointer<Ast::ArrayDefNode>& node);
